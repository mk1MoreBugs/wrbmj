...
def test__create_note__create_note_and_return_note_from_db__return_note(db: Session, unique_usernames: str) -> None:
    ...
    assert test_note.id == test_note_in_db.id
    assert test_note.noteContent == test_note_in_db.noteContent
    assert test_note.title_name == test_note_in_db.title_name
    assert test_note.last_update == test_note_in_db.last_update


def test__get_note_by_id__get_note_by_non_existent_id__return_None(db: Session, unique_usernames: str) -> None:
    raise Exception("Non implemented!")


def test__create_note__create_many_notes_and_return_notes_from_db__return_notes(db: Session, unique_usernames: str) -> None:
    username = get_unique_username(unique_usernames=unique_usernames)
    test_user = create_test_user(db=db, username=username, plain_password="plain_password")
    test_note_content = "note content"
    test_notes = [
...
def test__create_note__create_note_for_many_users__return_notes_of_correct_user(db: Session, unique_usernames: str) -> None:
    raise Exception("Non implemented!")
