...
from app.api.utils.redis_utils import set_note_in_redis, get_note_in_redis, get_and_delete_note_in_redis
...
async def test__get_note_in_redis__set_note_in_redis_and_get_it_in_redis__return_note_model(redis: Redis):
    ...
    await set_note_in_redis(redis=redis, note_id=note_id, note_in_detailed=test_note)
...
@pytest.mark.asyncio
async def test__delete_note_in_redis__set_note_in_redis_and_delete_it_in_redis__return_note_model(redis: Redis):
    note_id = 1
    test_note = NoteOutInDetailed(
        id=note_id,
        last_update=datetime.datetime.now(),
    ...
    await set_note_in_redis(redis=redis, note_id=note_id, note_in_detailed=test_note)
    note: NoteOutInDetailed = await get_and_delete_note_in_redis(redis=redis, note_id=note_id)
    print(note)
