...
def test_login_login_with_non_exist_user_return_exception(db: Session, client: TestClient, unique_usernames: str) -> None:
    username = get_unique_username(unique_usernames=unique_usernames)
...
def test_login_login_with_incorrect_password_return_exception(db: Session, client: TestClient, unique_usernames: str) -> None:
    username = get_unique_username(unique_usernames=unique_usernames)
    create_test_user(db=db, username=username, plain_password="plain_password")
    response = get_auth_token_response(
        client=client,
